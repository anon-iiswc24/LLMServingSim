...
def generateText(batch, parallel, npu_group):
    ...
    if not batch.isORCA:
        ...
        print(' '.join(["./run.sh", model, batch_size, input_len, init_cnt, parallel, npu_group, orca]))
        stdout = subprocess.run(["./run.sh", model, batch_size, input_len, init_cnt, parallel, npu_group, orca], text=True, capture_output=True)
        compile, simulate = getTime(stdout)
